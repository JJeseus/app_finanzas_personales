...
export default function CuentasPage() {
  ...
  // ✅ FIX: async + await (no Promises en state)
  const loadAccounts = async () => {
    try {
      ...
      const entries = await Promise.all(
        safeAccs.map(async (acc) => {
          const balance = await accountService.getBalance(acc.id)
          return [acc.id, balance] as const
        })
      )
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingAccount) {
        await accountService.update(editingAccount.id, formData as any)
        toast({
          title: 'Cuenta actualizada',
          description: 'La cuenta se ha actualizado correctamente.',
        })
      } else {
        await accountService.create(formData as any)
        toast({
          title: 'Cuenta creada',
