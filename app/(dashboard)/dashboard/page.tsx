...
export default function DashboardPage() {
  ...
  // ✅ FIX: async + await + Promise.all (no Promises en state)
  const loadData = async () => {
    try {
      const [
        ...
        dashboardService.getMonthlyFlowData(),
  ...
  // ✅ FIX: submit async + await + recarga
  ...
  return (
    <div className="flex flex-col gap-4 sm:gap-6 p-4 sm:p-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <KPICard title="Saldo Actual" value={formatCurrency(stats.currentBalance)} icon="balance" />
        <KPICard title="Ingresos del Mes" value={formatCurrency(stats.monthlyIncome)} icon="income" trend="up" />
        <KPICard title="Gastos del Mes" value={formatCurrency(stats.monthlyExpenses)} icon="expense" trend="down" />
        <KPICard
